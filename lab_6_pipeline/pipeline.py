...
class CorpusManager:
    ...
    def _validate_dataset(self) -> None:
        ...
        raw_files = [file for file in self.path_to_raw_txt_data.glob("*_raw.txt")]

        raw_ids = [file.name.split("_raw")[0] for file in raw_files]
        
        raw_ids = sorted([int(el) for el in raw_ids if el.isdigit()])
        ...
        meta_ids = sorted([int(el) for el in meta_ids if el.isdigit()])
    ...
    def _scan_dataset(self) -> None:
        ...
        for path in self.path_to_raw_txt_data.iterdir():

            if path.name.endswith("_raw.txt"):
                id = int(path.name.split("_raw")[0])
                self._storage[id] = from_raw(path, self._storage.get(id))

            if path.name.endswith("_meta.json"):
                id = int(path.name.split("_meta")[0])
                self._storage[id] = from_meta(path, self._storage.get(id))
...
class TextProcessingPipeline(PipelineProtocol):
    ...
    def run(self) -> None:
        ...
        for article in self._corpus.get_articles().values():
            analysis = self._analyzer.analyze([article.text])
            article.set_conllu_info(analysis[0])
...
class UDPipeAnalyzer(LibraryWrapper):
    ...
    def _bootstrap(self) -> Language:
        ...
        model = spacy_udpipe.load_from_path(lang="ru",
                                  path=str(model_path),
                                  meta={"description": "Syntagrus model"})
